  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 py-10 fade-in-up">
      ...
      <nav className="flex items-center gap-2 text-xs text-white/30 mb-6">
        <Link href="/videos" className="hover:text-naruto-orange transition-colors">← Fan-Hub Vidéo</Link>
        <span>/</span>
        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ background: `${color}22`, color }}>
          {CATEGORY_ICONS[video.category as VideoCategory]} {CATEGORY_LABELS[video.category as VideoCategory]}
        </span>
      </nav>
      ...
      <div className="flex flex-col sm:flex-row items-start gap-4">
        <div className="flex-1">
          <h1 className="text-xl sm:text-2xl font-bold text-white leading-snug mb-2">{video.title}</h1>
        ...
        <VideoPageLikeButton videoId={video.id} initialLikesCount={video.likesCount} color={color} />
      </div>
