...
export default async function ProfilePage({
  ...
  if (!user.consentGiven) {
    return <ConsentPage userId={user.id} />;
  }
    ...
    <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 py-12 fade-in-up space-y-8">
      ...
      <div className="relative rounded-3xl border border-white/8 bg-white/3 overflow-hidden">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at top left, rgba(255,102,0,0.12) 0%, transparent 55%)",
          }}
        />
        <div className="relative z-10 p-6 sm:p-8 flex flex-col sm:flex-row items-center sm:items-start gap-6">
          <div className="shrink-0">
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-2 border-naruto-orange/40 shadow-[0_0_20px_rgba(255,102,0,0.15)]">
              {avatarSrc ? (
                ...
                <div className="w-full h-full bg-naruto-orange/15 flex items-center justify-center text-xl font-black text-naruto-orange">
                  {initials}
                </div>
              )}
            </div>
          ...
          <div className="flex-1 text-center sm:text-left min-w-0">
            ...
            <div className="flex flex-wrap justify-center sm:justify-start gap-x-4 gap-y-1 mt-3 text-xs text-white/30">
              ...
              <span>
                ❤️ {likedCount} vidéo{likedCount !== 1 ? "s" : ""} aimée
                {likedCount !== 1 ? "s" : ""}
              </span>
            </div>
            <div className="mt-3">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {likedVideos.map(({ video }) => {
              const color =
                CATEGORY_COLORS[video.category as VideoCategory] ?? "#ff6600";
              return (
                <a
                  key={video.id}
                  href={`/videos/${video.id}`}
                  className="group flex flex-col overflow-hidden rounded-xl border border-white/8 bg-white/3 hover:border-white/15 transition-colors"
                >
                  ...
                  <p className="text-[11px] font-medium text-white/75 line-clamp-2 p-2.5 leading-snug">
                    {video.title}
                  </p>
                </a>
              );
      ...
      <div className="rounded-2xl border border-red-500/15 bg-red-500/4 p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            <button
              type="submit"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-white/6 text-white/60 border border-white/10 hover:bg-white/10 hover:text-white transition-all cursor-pointer"
            >
              Se déconnecter
            </button>
          </form>
    ...
    <div className="flex flex-1 items-center justify-center px-4 py-20 fade-in-up">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          <div className="w-14 h-14 rounded-2xl bg-naruto-orange/10 border border-naruto-orange/20 flex items-center justify-center text-2xl mx-auto mb-4">
            🍥
          </div>
          <h1 className="text-2xl font-black text-white mb-2">Bienvenue !</h1>
          <p className="text-white/45 text-sm">
            Avant de continuer, nous avons besoin de ton accord.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-white/8 bg-white/3 p-5 space-y-3 text-sm text-white/55 leading-relaxed">
          ...
          <ul className="space-y-2">
            <li className="flex items-start gap-2">
              <span className="text-naruto-orange mt-0.5">•</span>
              Ton <strong className="text-white/70">nom et e-mail</strong>{" "}
              Google
            </li>
            <li className="flex items-start gap-2">
              <span className="text-naruto-orange mt-0.5">•</span>
              Ta <strong className="text-white/70">photo de profil</strong>
            </li>
          </ul>
        </div>
    ...
    <div className="flex flex-1 items-center justify-center px-4 py-20 fade-in-up">
      <div className="w-full max-w-sm space-y-6">
        ...
        >
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-3 px-5 py-3.5 rounded-xl border border-white/12 bg-white/5 hover:bg-white/9 text-white font-semibold text-sm transition-all cursor-pointer"
          >
