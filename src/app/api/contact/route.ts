...
export async function POST(req: Request) {
  ...
  let body: { name?: string; email?: string; message?: string; captchaToken?: string };
  try {
  ...
  if (TURNSTILE_SECRET) {
    if (!captchaToken) {
      return NextResponse.json({ error: "Vérification de sécurité manquante." }, { status: 400 });
    }
    const ip =
  ...
  const discordRes = await fetch(DISCORD_CONTACT_WEBHOOK_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(buildDiscordPayload(name.trim(), email.trim(), message.trim())),
  });
